...
loc = pd.read_csv("location2.csv", names =['x','y'], header=None)
...
for m_i in range(len(m_num)):
    x =  data.loc[a[start[m_i]:end[m_i]],'x'].values
    y =  data.loc[a[start[m_i]:end[m_i]],'y'].values
    labels = 'Driver' + str(m_i)
    line, = ax.plot(x, y, marker=markers[m_i % 13], label=labels)#'Driver'+str(m_i))
    lines.append(line)
    out = np.array([x,y,labels])
    #print(type(x))
    #print(type(y))
    #print([x,y])
    #print(out)
    #print(out)
    #np.savetxt('out.csv', out, delimiter=',', fmt='%d')
